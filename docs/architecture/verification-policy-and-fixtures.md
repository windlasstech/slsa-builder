...
| `windlass.verify.error.npm-oidc-exchange-indeterminate`                    | The npm OIDC token exchange surface is unreadable or erroring, including HTTP `5xx`, a malformed response, or a success body violating the pinned exchange response contract (an unknown or missing member, a wrong `token_type`, or an invalid or inverted timestamp), so trusted-publisher configuration cannot be classified; the run fails as `indeterminate` even though the exchange mints only a short-lived publish token and mutates nothing. |
...
registry mutation because the exchange only mints a short-lived publish token. The set must also pin
the ADR 0081 success-response contract: an exchange response in the observed shape (exactly the four
documented members with epoch-second numeric `created` and `expires`) and one in the documented
shape (RFC 3339 string `created` and `expires`) must each be accepted; an unknown or missing member,
a `token_type` other than `"oidc"`, an inverted lifetime (`expires` not later than `created`), an
already-expired `expires` at exchange time, and a malformed timestamp value must each be rejected
before registry mutation with `npm-oidc-exchange-indeterminate`. No fixture may log or persist token
material. The set must also include an id-token env-absence capability probe in which a job without
`id-token: write` observes an absent `ACTIONS_ID_TOKEN_REQUEST_TOKEN` and fails with
`oidc-capability-unavailable` before any mutation. A filename cross-check mismatch fixture must
prove that the final path segment of the OIDC `workflow_ref` claim disagrees with the caller-scoped
`github.workflow_ref` context and fails with `trusted-publisher-mismatch` before signing and
registry mutation.
